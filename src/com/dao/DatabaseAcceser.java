...
public class DatabaseAcceser {
            ...
            while (fileScanner.hasNext()) {
                ...
                if (actualLine.equalsIgnoreCase(DATABASE_TOKEN)) {
                    ...
                    if (actualLine.contains(DATABASE_PASSWORD)) {
                        addConfigurationValue(DATABASE_PASSWORD,getLineValue(actualLine),databaseValues);
                    } else if (actualLine.contains(DATABASE_USERNAME)) {
                        addConfigurationValue(DATABASE_USERNAME,getLineValue(actualLine),databaseValues);
                    } else if (actualLine.contains(DATABASE_PORT)) {
                        addConfigurationValue(DATABASE_PORT,getLineValue(actualLine),databaseValues);
                    } else if (actualLine.contains(DATABASE_HOST)) {
                        addConfigurationValue(DATABASE_HOST,getLineValue(actualLine),databaseValues);
                    } else if (actualLine.contains(DATABASE_NAME)) {
                        addConfigurationValue(DATABASE_NAME,getLineValue(actualLine),databaseValues);
                    }
                }
            }
    ...
    private void addConfigurationValue(
        ...
        Hashtable<String,String> inoutConfigurationTable
            ...
            inoutConfigurationTable.put(inConfigurationKey,inConfigurationValue);
    }
